  ...
  <div class="space-y-4">
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <div v-else-if="!items || items.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">Aucun(e) {{lc}}</h3>
...
const { items, isLoading, error } = storeToRefs({{lc}}ListStore);
const { deleted: deletedItem } = storeToRefs({{lc}}DeleteStore);

const data = await useFetchList<{{titleUcFirst}}>("{{name}}");
{{lc}}ListStore.setData(data);

async function handleDelete(item: {{titleUcFirst}}) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    const { error: delError } = await useDeleteItem(item);
    if (!delError.value) {
      {{lc}}ListStore.deleteItem(item);
      {{lc}}DeleteStore.setDeleted(item);
      emit("deleted", item);
    }
  }
}
