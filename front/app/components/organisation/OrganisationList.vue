  ...
  <div class="space-y-4">
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <div v-else-if="!items || items.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">Aucun(e) organisation</h3>
          ...
          <tbody class="divide-y divide-neutral-200 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
            ...
            >
              <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-mono text-neutral-500">
                <NuxtLink
                  :to="`/organisations/${getIdFromIri(item['@id'])}`"
              ...
              <td class="whitespace-nowrap px-3 py-4 text-sm text-neutral-500">
                <UBadge color="neutral" variant="subtle" size="xs">
                  {{ item.projects?.length || 0 }} projet{{ (item.projects?.length || 0) > 1 ? 's' : '' }}
                </UBadge>
              </td>
...
const { deleted: deletedItem } = storeToRefs(organisationDeleteStore);

const data = await useFetchList<Organisation>("organisations");
organisationListStore.setData(data);

async function handleDelete(item: Organisation) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    const { error: delError } = await useDeleteItem(item);
    if (!delError.value) {
      organisationListStore.deleteItem(item);
      organisationDeleteStore.setDeleted(item);
      emit("deleted", item);
    }
  }
}
