  ...
  <div class="space-y-4">
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <div v-else-if="!items || items.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">Aucun(e) organisationmember</h3>
          ...
          <tbody class="divide-y divide-neutral-200 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
            ...
            >
              <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-mono text-neutral-500">
                <NuxtLink
                  :to="`/organisationmembers/${getIdFromIri(item['@id'])}`"
...
import { useOrganisationMemberListStore } from "~/stores/organisationmember/list";
...
const emit = defineEmits<{
  (e: "create"): void;
  (e: "edit", item: OrganisationMember): void;
  (e: "show", item: OrganisationMember): void;
  (e: "deleted", item: OrganisationMember): void;
}>();

const organisationmemberListStore = useOrganisationMemberListStore();
const organisationmemberDeleteStore = useOrganisationMemberDeleteStore();

const { items, isLoading, error } = storeToRefs(organisationmemberListStore);
const { deleted: deletedItem } = storeToRefs(organisationmemberDeleteStore);

const data = await useFetchList<OrganisationMember>("organisation_members");
organisationmemberListStore.setData(data);

async function handleDelete(item: OrganisationMember) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    const { error: delError } = await useDeleteItem(item);
    if (!delError.value) {
      organisationmemberListStore.deleteItem(item);
      organisationmemberDeleteStore.setDeleted(item);
      emit("deleted", item);
    }
  }
}
...
onBeforeUnmount(() => {
  organisationmemberListStore.$reset();
