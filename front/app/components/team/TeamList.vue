  ...
  <div class="space-y-4">
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <div v-else-if="!items || items.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">Aucun(e) team</h3>
...
import { useTeamListStore } from "~/stores/team/list";
...
const emit = defineEmits<{
  (e: "create"): void;
  (e: "edit", item: Team): void;
  (e: "show", item: Team): void;
...
const { items, isLoading, error } = storeToRefs(teamListStore);
const { deleted: deletedItem } = storeToRefs(teamDeleteStore);

const data = await useFetchList<Team>("teams");
teamListStore.setData(data);

async function handleDelete(item: Team) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    const { error: delError } = await useDeleteItem(item);
    if (!delError.value) {
      teamListStore.deleteItem(item);
      teamDeleteStore.setDeleted(item);
      emit("deleted", item);
    }
  }
}
