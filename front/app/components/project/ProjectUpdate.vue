...
const emit = defineEmits<{
  (e: "updated", item: Project): void;
  (e: "deleted", item: Project): void;
  (e: "cancel"): void;
}>();

const projectUpdateStore = useProjectUpdateStore();
const projectDeleteStore = useProjectDeleteStore();
const { error: deleteError, isLoading: deleteLoading } = storeToRefs(projectDeleteStore);
const { updated, violations, isLoading, error } = storeToRefs(projectUpdateStore);
...
async function loadItem() {
  ...
  if (!props.id) return;
  const data = await useFetchItem<Project>(`projects/${props.id}`);
  item.value = data.retrieved.value;
  projectUpdateStore.setData(data);
}
...
async function update(payload: Project) {
  if (!item.value) return;

  const data = await useUpdateItem<Project>(item.value, payload);
  projectUpdateStore.setUpdateData(data);
...
async function deleteItem() {
  ...
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    ...
    if (delError.value) {
      projectDeleteStore.setError(delError.value);
      return;
    }

    projectDeleteStore.setLoading(Boolean(delLoading?.value));
    projectDeleteStore.setDeleted(item.value);
    emit("deleted", item.value);
  }
}

onBeforeUnmount(() => {
  projectUpdateStore.$reset();
