<template>
  <UCard>
    ...
    <div v-if="item" class="divide-y divide-neutral-200 dark:divide-neutral-800">
      ...
      <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
        <dt class="text-sm font-medium text-neutral-500 capitalize">webserverUrl</dt>
        <dd class="mt-1 text-sm text-neutral-900 dark:text-neutral-100 sm:col-span-2 sm:mt-0">
          {{ item.webserverUrl }}
        </dd>
      </div>
      ...
      <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
        <dt class="text-sm font-medium text-neutral-500 capitalize">stagings</dt>
        <dd class="mt-1 text-sm text-neutral-900 dark:text-neutral-100 sm:col-span-2 sm:mt-0">
          {{ item.stagings }}
        </dd>
      </div>
...
async function load() {
  ...
  try {
    const data = await useFetchItem<DeploymentServer>(`deployment_servers/${idToLoad}`);
...
function goBack() {
  emit("back");
  router.push({ path: "/deploymentservers" });
}
