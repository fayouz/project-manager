  ...
  <div class="space-y-4">
    ...
    <UCard :ui="{ body: 'p-0 sm:p-0' }">
      ...
      <div v-else-if="!items || items.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">Aucun(e) teammember</h3>
          ...
          <tbody class="divide-y divide-neutral-200 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
            ...
            >
              <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-mono text-neutral-500">
                <NuxtLink
                  :to="`/teammembers/${getIdFromIri(item['@id'])}`"
...
import { useTeamMemberListStore } from "~/stores/teammember/list";
...
const emit = defineEmits<{
  (e: "create"): void;
  (e: "edit", item: TeamMember): void;
  (e: "show", item: TeamMember): void;
  (e: "deleted", item: TeamMember): void;
}>();

const teammemberListStore = useTeamMemberListStore();
const teammemberDeleteStore = useTeamMemberDeleteStore();

const { items, isLoading, error } = storeToRefs(teammemberListStore);
const { deleted: deletedItem } = storeToRefs(teammemberDeleteStore);

const data = await useFetchList<TeamMember>("team_members");
teammemberListStore.setData(data);

async function handleDelete(item: TeamMember) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet élément ?")) {
    const { error: delError } = await useDeleteItem(item);
    if (!delError.value) {
      teammemberListStore.deleteItem(item);
      teammemberDeleteStore.setDeleted(item);
      emit("deleted", item);
    }
  }
}
